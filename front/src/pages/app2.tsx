...
type Page = 'home' | 'product' | 'cart' | 'vendor-dashboard' | 'client-dashboard' | 'wallet' | 'locations' | 'search';
...
export default function App() {
  ...
  const handleAddToCart = async (productId: string) => {
    ...
    // In a real implementation, you would save this to the user's cart
    // For now, we just increment the counter
    if (currentUser?.accessToken) {
      try {
        // Could save to user's cart in backend
  ...
  const handleTransferSuccess = (amount: number, method: string, recipient: string) => {
    ...
    // Dans un vrai système, on déduirait selon la méthode choisie
  };
  ...
  if (currentPage === 'vendor-dashboard') {
    return <VendorDashboard onBack={navigateToHome} accessToken={currentUser?.accessToken} />;
  }

  if (currentPage === 'client-dashboard' && currentUser) {
    return <ClientDashboard onBack={navigateToHome} user={currentUser} />;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <RechargeWalletModal
        ...
        onRechargeSuccess={handleGlobalWalletRecharge}
