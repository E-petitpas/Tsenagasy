...
const AdminDashboard = () => {
  ...
  const sponsors = [
   ...
   useEffect(() => {
    const fetchAccounts = async () => {
      try {
        const { data } = await axios.get(`${API_BASE_URL}/getAllUser`);
        console.log("Response API:", data);
        // Transformer les données pour correspondre à ce que tu attends dans le tableau
        const formattedUsers = data.users.map((user : any) => ({
          id: user.id,
          name: user.nom,
          email: user.email,
          role: user.role,
          status: 'pending', // par défaut si tu n’as pas le status
          joinDate: 'N/A'    // ou formater avec une vraie date si dispo
        }));
        setAccounts(formattedUsers);
        console.log("comptes ", formattedUsers);
      } catch (error) {
        console.error('Erreur fetch accounts:', error);
      }
    };
  ...
  try {
    await axios.post(`${API_BASE_URL}/api/acceptUser/${userId}`);
...
const handleReject = async (userId: number) => {
  try {
    await axios.post(`${API_BASE_URL}/api/rejectUser/${userId}`);
  ...
  <div className="flex justify-between items-center mb-6">
    ...
    <button className="bg-[#2D8A47] text-white px-4 py-2 rounded-lg hover:bg-[#245A35] transition-colors flex items-center gap-2" onClick={() => setIsAddUserModalOpen(true)}>
      <Plus size={18} /> Nouveau compte
              </button>
              {isAddUserModalOpen && (
  <AddUserModal
    onClose={() => setIsAddUserModalOpen(false)}
    onUserAdded={(newUser) => setAccounts(prev => [newUser, ...prev])}
  />
)}
              ...
              <td className="px-4 py-3 text-sm text-gray-900 capitalize">
                <select
                  ...
                  onChange={async (e) => {
                    ...
                    try {
                      await axios.post(`${API_BASE_URL}/api/changeUserRole/${account.id}`, { role: newRole });
                      setAccounts(prev =>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-gradient-to-br from-[#2D8A47] to-[#245A35] text-white rounded-lg p-6">
                ...
                <div className="bg-gradient-to-br from-[#FFA726] to-[#F57C00] text-white rounded-lg p-6">
                ...
                <div className="bg-gradient-to-br from-[#FFD700] to-[#FFC107] text-white rounded-lg p-6">
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header avec couleur vert malgache */}
      <header className="bg-[#2D8A47] text-white shadow-lg">
        <div className="px-6 py-4">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-white text-opacity-90 mt-1">Tsena.mg - Plateforme d'administration</p>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-sm font-medium">Admin Principal</p>
                <p className="text-xs text-white text-opacity-75">admin@tsena.mg</p>
              </div>
        ...
        {/* Navigation tabs avec couleurs malgaches */}
        <nav className="px-6 border-t border-white border-opacity-20">
          <div className="flex gap-1 overflow-x-auto">
            {[
              { id: 'overview', label: 'Vue d\'ensemble', icon: <BarChart3 size={18} /> },
              { id: 'accounts', label: 'Comptes', icon: <Users size={18} /> },
              { id: 'products', label: 'Produits', icon: <Package size={18} /> },
              { id: 'sponsors', label: 'Sponsors', icon: <Star size={18} /> },
              { id: 'analytics', label: 'Analytiques', icon: <TrendingUp size={18} /> },
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-3 font-medium transition-colors whitespace-nowrap ${
                  activeTab === tab.id
                    ? 'bg-white text-[#2D8A47] rounded-t-lg'
